...
public class Recommend_review extends JFrame {
	...
	public Recommend_review() {
		setLayout(new BorderLayout());
		JPanel Pan = new JPanel();
		String s = "리뷰의 개수로 산출한 결과입니다.  "; // 멘트
		...
		table.getColumn("리뷰 수").setPreferredWidth(200); // 가로너비
	...
	public void SQL() {
		...
		try {
			...
			if (combo.getSelectedIndex() == 0) {
				sql = "select movie_name,count(r.comment) from movie m left outer join review r on m.movie_number=r.movie_number group by m.movie_number order by count(r.comment) desc";

			} else {
				sql = "select movie_name,count(r.comment) from movie m left outer join review r on m.movie_number=r.movie_number group by m.movie_number order by count(r.comment)";
