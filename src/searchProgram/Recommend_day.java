...
public class Recommend_day extends JFrame {
	...
	public Recommend_day() {
		setLayout(new BorderLayout());
		JPanel Pan = new JPanel();
		String s = "개봉일 순으로 조회된 결과입니다.  "; // 멘트
		...
		table.getColumn("개봉일").setPreferredWidth(200); // 가로너비
	...
	public void SQL() {
		...
		try {
			...
			if (combo.getSelectedIndex() == 0) {
				sql = "select movie_name, day from movie order by day desc";

			} else {
				sql = "select movie_name, day from movie order by day asc";
